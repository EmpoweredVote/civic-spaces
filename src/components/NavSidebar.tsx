...
interface NavSidebarProps {
  ...
  onOpenModQueue: () => void
  theme: 'light' | 'dark'
  /** Real place/county names resolved directly from the address geocoder —
   *  preferred over the separate (less reliable) Census data API lookup
   *  useJurisdictionName falls back to. */
...
const GEO_TABS: { key: TabKey; label: string; icon: string }[] = [
  { key: 'neighborhood', label: 'Local', icon: 'home' },
  { key: 'local', label: 'County', icon: 'building' },
...
function GeoNavItem({
  ...
  // Hook is always called (rules of hooks) even when its result ends up
  // unused below, e.g. when previewGeo already has a direct name.
  const fetchedName = useJurisdictionName(slice, tab.label)

  let label = tab.label
  if (tab.key === 'neighborhood') {
    label = previewGeo?.placeName ?? fetchedName
  } else if (tab.key === 'local') {
    label = previewGeo?.countyName ?? fetchedName
  }
...
export default function NavSidebar({ variant = 'pinned', ...props }: NavSidebarProps) {
  if (variant === 'drawer') {
    return (
      <nav className="flex flex-col gap-6 p-4 overflow-y-auto contain-paint h-full" aria-label="Civic space navigation">
  ...
  return (
    <nav
      className="hidden lg:flex flex-col gap-6 p-4 rounded-2xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm overflow-y-auto contain-paint"
