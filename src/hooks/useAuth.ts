...
const LOGIN_URL = `https://accounts.empowered.vote/login?redirect=${encodeURIComponent(window.location.origin)}`
...
/**
 * Opt-in local dev session (?dev=1) for previewing the authenticated
 * experience — the default local visit is anonymous, matching a real
 * logged-out production visitor. Never runs in a production build; real
 * SSO/localStorage session checks above always take priority.
 */
...
export function useAuth(): AuthState & { loginUrl: string } {
  ...
  useEffect(() => {
    async function resolveAuth() {
      // Local dev defaults to the same logged-out state a real production
      // visitor gets — visit with ?dev=1 to preview the authenticated
      // (posting/replying) experience instead.
      const wantsDevSession = import.meta.env.DEV && new URLSearchParams(window.location.search).get('dev') === '1'
      ...
      if (stored) {
        if (!isTokenExpired(stored)) {
          ...
          // A previously auto-created dev-guest session only counts when
          // ?dev=1 is explicitly requested — otherwise this visit is
          // treated as anonymous, matching a real logged-out visitor.
          if (userId && (userId !== DEV_GUEST_ID || wantsDevSession)) {
            setAuthState({ userId, isAuthenticated: true, isLoading: false })
            return
          }
          if (userId === DEV_GUEST_ID && !wantsDevSession) {
      ...
      // 4. Local dev opt-in session — only when ?dev=1 is requested.
      if (wantsDevSession) {
        const userId = storeToken(createDevFallbackToken())
        if (userId) {
          setAuthState({ userId, isAuthenticated: true, isLoading: false })
          return
        }
